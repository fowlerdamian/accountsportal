...
import {
  ...
  type SourceBase, type AnalyticsData, type HubspotData,
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        <Mini label="Active users" value={fmtNum(d.activeUsers)} />
        <Mini label="Sessions" value={fmtNum(d.sessions)} />
        <Mini label="Page views" value={fmtNum(d.pageViews)} />
        <Mini label="Key events" value={fmtNum(d.keyEvents)} />
      </div>
      {!!d.timeseries?.length && (
        <ResponsiveContainer width="100%" height={140}>
          <AreaChart data={d.timeseries} margin={{ top: 4, right: 8, left: -18, bottom: 0 }}>
            <defs>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <StatCard icon={BarChart3} accent={palette.purple} label="GA active users"
              value={data.analytics.ok ? fmtNum(data.analytics.activeUsers) : "—"}
              sub={data.analytics.ok ? `${fmtNum(data.analytics.sessions)} sessions · 28d` : "Action needed"} />
          </div>
