...
import { Plus, Loader2, Calendar, AlertCircle, Check, X as XIcon, Trash2, RotateCcw, ArrowUpDown } from "lucide-react";
            ...
            <div className="flex items-center gap-1">
              ...
              {activeStage.name === "Prototype" && (
                <span className={cn(
                  ...
                  activeStage.metadata?.ordered ? "bg-green-500" : "bg-red-500/80",
                )}>
                  {activeStage.metadata?.ordered
        ...
        <div className="space-y-1 opacity-60">
          <div className="h-1 rounded-full bg-muted overflow-hidden">
            <div
              className={cn(
                "h-full rounded-full",
                burnPct >= 100 ? "bg-red-500" : burnPct >= 80 ? "bg-amber-500" : "bg-primary/60",
              )}
              style={{ width: `${burnPct}%` }}
            />
          </div>
      ...
      <div className="flex items-center justify-between mt-auto">
        ...
        {isProduct && (
          <span className="text-[10px] text-muted-foreground/50">
            Stage {Math.max(stageIdx + 1, 1)} of {NEW_PRODUCT_STAGES.length}
          </span>
        )}
      </div>
    </div>
...
export default function ProjectsList() {
  ...
  const activeStageByProject = Object.fromEntries(
    activeStages.map(s => [s.project_id, s])
  );

  // Type-filtered projects
  const filteredByType = typeFilter === "all"
    ? projects
    : typeFilter === "other"
    ? projects.filter(p => p.type !== "new_product" && p.type !== "web")
    : projects.filter(p => p.type === typeFilter);

  // Stage sub-filter (only relevant when showing new_product)
  const filteredByStage = (typeFilter === "new_product" || typeFilter === "all") && stageFilter !== "all"
    ? filteredByType.filter(p => {
        if (p.type !== "new_product") return true; // non-products pass through
        return activeStageByProject[p.id]?.name === stageFilter;
      })
    : filteredByType;

  const filtered = sortByPriority
    ? [...filteredByStage].sort((a, b) => (b.priority_score ?? 0) - (a.priority_score ?? 0))
    : filteredByStage;
  ...
  return (
    <HubLayout>
      <div className="space-y-5 animate-fade-in">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            <FilterPill active={typeFilter === "new_product"} onClick={() => { setTypeFilter("new_product"); setStageFilter("all"); }}>
              New Products <span className="ml-1 opacity-50">{projects.filter(p => p.type === "new_product").length}</span>
            </FilterPill>
        ...
        {/* ── Stage sub-filter (for new products) ── */}
        {showStageFilter && productProjects.length > 0 && !showBin && (
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {NEW_PRODUCT_STAGES.map((s, i) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "px-3 py-0.5 rounded-full text-xs font-medium transition-colors border flex items-center gap-1.5",
                    stageFilter === s
                      ? "bg-muted text-foreground border-border"
                      : "text-muted-foreground border-transparent hover:border-border/50",
                  )}
        ...
        {showBin && (
          <>
            <p className="text-xs text-muted-foreground">
              Projects are permanently deleted after 15 days.
            </p>
