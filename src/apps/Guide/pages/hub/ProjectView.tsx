...
import { HubLayout, useHub } from "@guide/components/hub/HubLayout";
import { StatsCard } from "@guide/components/admin/StatsCard";
import { StatusPill, TASK_STATUS_ORDER, nextTaskStatus } from "@guide/components/hub/StatusPill";
import { PriorityPill } from "@guide/components/hub/PriorityPill";
import { ContractorAvatar } from "@guide/components/hub/ContractorAvatar";
import { ActivityFeed } from "@guide/components/hub/ActivityFeed";
import { TaskDrawer } from "@guide/components/hub/TaskDrawer";
import { LogTimeForm } from "@guide/components/hub/LogTimeForm";
...
function ProjectViewContent() {
  ...
  async function handleStatusChange(status: ProjectStatus) {
    ...
    try {
      ...
      sendNotification({ type: "task_status", task_title: project.name, status, author: authorName, project_name: project.name });
  ...
  async function handleStatusCycle(task: Task, e: React.MouseEvent) {
    ...
    try {
      ...
      sendNotification({ type: "task_status", task_title: task.title, status: next, author: authorName, project_name: project?.name ?? "" });
    } catch { toast.error("Failed to update status"); }
  }
  ...
  async function handlePostActivity() {
    ...
    try {
      ...
      sendNotification({ type: "activity_posted", author: authorName, project_name: project?.name ?? "", content: activityInput });
